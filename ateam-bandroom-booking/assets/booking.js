(function () {
    ...
    function defaultEndTime() { return timeFromMinutes(Math.min(minutes(config.openingTime) + 60, minutes(config.closingTime))); }
    ...
    fill(end, defaultEndTime());
    date.min = config.currentDate || new Date().toISOString().slice(0, 10);
    calculate();
    ...
    form.addEventListener('submit', function (event) {
        ...
        fetch(config.ajaxUrl, { method: 'POST', body: data, credentials: 'same-origin' })
            .then(response => response.json()).then(response => {
                ...
                if (response.success) { form.reset(); fill(start, config.openingTime); fill(end, defaultEndTime()); calculate(); }
