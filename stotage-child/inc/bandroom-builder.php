...
final class ATeam_Bandroom_Builder {
    ...
    private function defaults(): array {
        return array(
            ...
            'booking_url' => '#ateam-bandroom-booking',
            ...
            'cta_url' => '#ateam-bandroom-booking',
            'cta_bg_image_id' => 0,
            'features' => array(
    ...
    public function render_frontend(): void {
        ...
        echo '<div class="ateam-bandroom-pricing"><div class="ateam-bandroom-dollar">$</div><div class="ateam-bandroom-rate"><strong>$' . esc_html($s['price_one']) . '</strong><span>' . esc_html($s['price_one_label']) . '</span></div><div class="ateam-bandroom-divider"></div><div class="ateam-bandroom-rate"><strong>$' . esc_html($s['price_two']) . '</strong><span>' . esc_html($s['price_two_label']) . '</span></div><div class="ateam-bandroom-booking">' . $this->button_html($s['booking_url'], $s['booking_label'], '<i class="far fa-calendar-alt"></i> ') . '<p>' . esc_html($s['pricing_note']) . '</p></div></div>';
        ...
        echo '</div><section class="ateam-bandroom-cta"' . ($cta ? ' style="--bandroom-cta:url(' . esc_url($cta) . ')"' : '') . '><div><h2>' . esc_html($s['cta_title']) . '</h2><p>' . esc_html($s['cta_text']) . '</p></div>' . $this->button_html($s['cta_url'], $s['cta_label'], '', ' <span>&rarr;</span>') . '</section></div></section></div>';
    }

    private function button_html(string $url, string $label, string $before = '', string $after = ''): string {
        $classes = '#ateam-bandroom-booking' === $url ? ' class="ateam-bandroom-booking-trigger"' : '';
        return '<a href="' . esc_url($url) . '"' . $classes . '>' . $before . esc_html($label) . $after . '</a>';
    }
}
