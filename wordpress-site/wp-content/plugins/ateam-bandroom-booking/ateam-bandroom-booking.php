...
final class ATeam_Bandroom_Booking {
    ...
    public function enqueue_assets(): void {
        ...
        wp_enqueue_script('ateam-bandroom-booking', plugin_dir_url(__FILE__) . 'assets/booking.js', array(), '1.0.1', true);
    ...
    public function submit_booking(): void {
        ...
        $s = $this->settings();
        if (!$name || !$email || !$phone || !$this->valid_date($date) || !$start || !$end) {
            wp_send_json_error(array('message' => 'Please complete every required booking field.'), 400);
        }
        if ($date < current_time('Y-m-d') || !$this->available_time($start, $s) || !$this->available_time($end, $s)) {
