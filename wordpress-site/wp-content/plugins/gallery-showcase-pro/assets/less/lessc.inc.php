...
class gspwp_lessc {
    ...
    protected function injectVariables($args) {
        $this->pushEnv();
        $parser = new gspwp_lessc_parser($this, __METHOD__);
    ...
    protected function makeParser($name) {
        $parser = new gspwp_lessc_parser($this, $name);
    ...
    protected function newFormatter() {
        $className = "gspwp_lessc_formatter_lessjs";
        if (!empty($this->formatterName)) {
            if (!is_string($this->formatterName))
                return $this->formatterName;
            $className = "gspwp_lessc_formatter_$this->formatterName";
        }
...
class gspwp_lessc_parser {
    ...
    public function __construct($lessc, $sourceName = null) {
        ...
        if (!self::$operatorString) {
            self::$operatorString = '(' . implode('|', array_map(array('gspwp_lessc', 'preg_quote'), array_keys(self::$precedence))) . ')';

            $commentSingle = gspwp_lessc::preg_quote(self::$commentSingle);
            $commentMultiLeft = gspwp_lessc::preg_quote(self::$commentMultiLeft);
            $commentMultiRight = gspwp_lessc::preg_quote(self::$commentMultiRight);
    ...
    protected function isDirective($dirname, $directives) {
        ...
        $pattern = implode("|", array_map(array("gspwp_lessc", "preg_quote"), $directives));
    ...
    protected function expressionList(&$exps) {
        ...
        $exps = gspwp_lessc::compressList($values, ' ');
        return true;
    }
    ...
    public function propertyValue(&$value, $keyName = null) {
        ...
        $value = gspwp_lessc::compressList($values, ', ');
        return true;
    }
    ...
    protected function openString($end, &$out, $nestingOpen = null, $rejectStrs = null) {
        ...
        $stop = array("'", '"', "@{", $end);
        $stop = array_map(array("gspwp_lessc", "preg_quote"), $stop);
...
protected function literal($what, $eatWhitespace = null) {
    ...
    if (!isset(self::$literalCache[$what])) {
        self::$literalCache[$what] = gspwp_lessc::preg_quote($what);
    }
...
protected function to($what, &$out, $until = false, $allowNewline = false) {
    ...
    if (!$this->match('(' . $validChars . '*?)' . gspwp_lessc::preg_quote($what), $m, !$until))
        return false;
    if ($until)
...
class gspwp_lessc_formatter_classic {
...
public function block($block) {
    ...
    if (!empty($block->selectors)) {
        ...
        if ($isSingle) {
            echo $this->openSingle; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
            $inner = "";
        } else {
            echo $this->open . $this->break; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
    ...
    if (!empty($block->lines)) {
        ...
        if (!$isSingle && !empty($block->children)) {
            echo $this->break; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        }
    }
    ...
    if (!empty($block->selectors)) {
        ...
        if ($isSingle) {
            echo $this->closeSingle . $this->break; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
        } else {
...
class gspwp_lessc_formatter_compressed extends gspwp_lessc_formatter_classic {
...
class gspwp_lessc_formatter_lessjs extends gspwp_lessc_formatter_classic {
