...
class Parsedown
{
    ...
    protected $safeLinksWhitelist = array('http://', 'https://', 'ftp://', 'ftps://', 'mailto:', 'tel:', 'data:image/png;base64,', 'data:image/gif;base64,', 'data:image/jpeg;base64,', 'irc:', 'ircs:', 'git:', 'ssh:', 'news:', 'steam:');
    #
    # Lines
    #
    ...
    protected function linesElements(array $lines)
    {
        ...
        foreach ($lines as $line) {
            ...
            while (($beforeTab = \strstr($line, "\t", \true)) !== \false) {
    ...
    protected function blockComment($Line)
    {
        ...
        if (\strpos($Line['text'], '<!--') === 0) {
            $Block = array('element' => array('rawHtml' => $Line['body'], 'autobreak' => \true));
    ...
    protected function blockHeader($Line)
    {
        ...
        $text = \trim($text, ' ');
        $Block = array('element' => array('name' => 'h' . $level, 'handler' => array('function' => 'lineElements', 'argument' => $text, 'destination' => 'elements')));
        return $Block;
    }
    #
    # List
    protected function blockList($Line, ?array $CurrentBlock = null)
    {
        ...
        if (\preg_match('/^(' . $pattern . '([ ]++|$))(.*+)/', $Line['text'], $matches)) {
            ...
            $markerWithoutWhitespace = \strstr($matches[1], ' ', \true);
            ...
            if ($name === 'ol') {
                $listStart = \ltrim(\strstr($matches[1], $Block['data']['markerType'], \true), '0') ?: '0';
                if ($listStart !== '1') {
    ...
    protected function blockMarkup($Line)
    {
        ...
        if (\preg_match('/^<[\/]?+(\w*)(?:[ ]*+' . $this->regexHtmlAttribute . ')*+[ ]*+(\/)?>/', $Line['text'], $matches)) {
            ...
            $Block = array('name' => $matches[1], 'element' => array('rawHtml' => $Line['text'], 'autobreak' => \true));
            return $Block;
        }
    }
    ...
    protected function blockTable($Line, ?array $Block = null)
    {
        ...
        # ~
        $Block = array('alignments' => $alignments, 'identified' => \true, 'element' => array('name' => 'table', 'elements' => array()));
    ...
    protected function inlineImage($Excerpt)
    {
        ...
        $Inline = array('extent' => $Link['extent'] + 1, 'element' => array('name' => 'img', 'attributes' => array('src' => $Link['element']['attributes']['href'], 'alt' => $Link['element']['handler']['argument']), 'autobreak' => \true));
