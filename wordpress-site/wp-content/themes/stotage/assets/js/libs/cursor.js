;(function ($) {
  ...
  const Cursor = (function() {
    ...
    function enterHandler({ target }) {
      ...
      // Nếu là IMG hoặc H1-H6 có cha .pxl-heading thì thêm has-glass
      if (
        target.tagName.toLowerCase() === 'img' ||
        (/^h[1-6]$/.test(target.tagName.toLowerCase()) && target.closest('.pxl-heading'))
      ) {
        cursor.classList.add('has-glass');
      }
    ...
    function leaveHandler({ target }) {
      ...
      cursor.classList.remove('has-glass'); // Xoá khi rời IMG/H1-H6
      label.innerHTML = '';
      drap.innerHTML = '';
      icon.innerHTML = '';
    }
    ...
    function update() {
      ...
      cursorTriggers = document.querySelectorAll([
        ...
        "a",
        "input",
        ".pxl-image-single img",   // thêm img vào triggers
        "h1","h2","h3","h4","h5","h6", // heading vào triggers
        "[data-cursor]",
        "[data-cursor-label]",
        "[data-cursor-drap]",
        "[data-drap-style]",
